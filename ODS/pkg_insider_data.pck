create or replace package pkg_insider_data as
  ...
  procedure load_affiliate_inventory
  (
    p_run_id       number,
    p_process_date number,
    p_process_name varchar2
...
create or replace package body pkg_insider_data as
  ...
  begin
    ...
    if upper(l_process_name) = 'PROCESS' then
      load_affiliate_inventory(l_run_id, l_process_date, l_process_name);
    end if;
  ...
  begin
    for i in (select date_id
              from   dim_date
              where  date_id >=
                     to_number(to_char(to_date(p_process_date, 'yyyymmdd') - 5, 'yyyymmdd'))
              and    date_id <= p_process_date
              order  by date_id) loop
      ---- load affiliate invnetory to WWW
      l_process_name := p_process_name || '-' ||
                        'affiliate_inventory@etlwww' || '_' ||
                        i.date_id;
      if pkg_log_process.is_not_complete(p_run_id, p_process_date, l_process_name) then
        pkg_log_process.log_process(p_run_id, p_process_date, l_process_name, 'I');
        insider_data.pkg_generic_ddl.truncate_partition@etlwww('insider_data', 'insider_affiliate_inventory', 'P_' ||
                                                                i.date_id);
        insert /*+ append */
        into insider_affiliate_inventory@etlwww
          (date_id, affiliate_id, imps_total,
           imps_qualified, imps_unqualified,
           unqualified_percent, qualified_percent)
          select date_id, affiliate_id,
                 sum(imps_total) as total,
                 sum(imps_preset_3372) as imps_qualified,
                 sum(imps_total) - sum(imps_preset_3372) as imps_unqualified,
                 ((sum(imps_total) - sum(imps_preset_3372)) /
                  sum(imps_total)) * 100 as unqualified_percent,
                 sum(imps_preset_3372) * 100 /
                  sum(imps_total) as qualified_percent
          from   analytics_data.affiliate_inventory@etladq
          where  date_id = i.date_id
          group  by date_id, affiliate_id;
        l_cnt := sql%rowcount;
        commit;
        insider_data.pkg_generic_ddl.analyze_partition@etlwww('insider_data', 'insider_affiliate_inventory', 'P_' ||
                                                               i.date_id);
        pkg_log_process.log_process(p_run_id, p_process_date, l_process_name, 'UC', l_cnt);
      end if;
    end loop;
